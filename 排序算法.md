...
Radix()
{
	Queue<int>[10] quene //10个队列
	for i = [0, a.max.tostring.length)//最大的数的位数
	{
		var index = Math.Pow(10, n)//10的n次方
		for j=[0,n)
		{
			int rem
			Math.DivRem(a[j]/index, 10, out mod)//取得某一位上的数
			quene[rem].Enquene(a[j])
		}
		int n = 0;
		foreach(q in quene)
			while(q.count != 0)
